...
class SoftmaxCrossEntropy(Criterion):
    ...
    def forward(self, x, y):
        ...
        N = self.logits .shape[0]  # TODO
        C = self.logits .shape[1] # TODO

        Ones_C = np.ones((C,1))  # TODO
        Ones_N = np.ones((N,1))  # TODO

        self.softmax = np.exp(self.logits )/np.sum(np.exp(self.logits ),axis =1).reshape(-1,1)  # TODO
        crossentropy = -(self.labels * np.log(self.softmax)) @ Ones_C  # TODO
