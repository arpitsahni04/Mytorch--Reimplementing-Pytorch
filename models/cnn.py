...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        out_chans = num_input_channels
        for i in range(self.nlayers):
            
           conv_layer =  Conv1d(in_channels=out_chans, out_channels=num_channels[i],
           ...
           outsize = (input_width-kernel_sizes[i])//strides[i] +1
           input_width= outsize
           out_chans = num_channels[i]
           self.convolutional_layers.append(conv_layer)
        
        
        self.flatten = Flatten()
        self.linear_layer = Linear(in_features =out_chans*outsize, out_features =num_linear_neurons )
